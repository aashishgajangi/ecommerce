...
function LoginForm() {
  ...
  const [error, setError] = useState(
    ...
    searchParams.get('error') === 'deactivated' ? 'Your account has been deactivated.' :
    searchParams.get('error') ? 'Sign-in failed. Please try again.' : ''
  )
  const [loading, setLoading] = useState(false)

  async function mergeGuestCartAndRedirect(token: string) {
    ...
    if (guestItems.length > 0) {
      try {
        ...
        clearGuestCart()
      } catch { /* silently ignore merge errors */ }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await mergeGuestCartAndRedirect(res.data.token)
    } catch (err: unknown) {
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
      ...
      <p className="text-sm text-gray-500 mb-6">Sign in to your Hangout Cakes account</p>
      ...
      <a
        ...
        className="mt-4 w-full flex items-center justify-center gap-2 border border-gray-200 rounded-full py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
      >
      ...
      <p className="mt-6 text-center text-sm text-gray-500">
        ...
        <Link href="/auth/register" className="text-rose-600 font-medium hover:underline">Register</Link>
      </p>
    </div>
  )
}
...
export default function LoginPage() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <Suspense fallback={<div className="w-full max-w-md h-96 bg-white rounded-2xl border border-gray-100 animate-pulse" />}>
        <LoginForm />
      </Suspense>
    </div>
  )
